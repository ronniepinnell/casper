...
Usage:
  python3 scripts/backfill.py [--repo owner/name] [--limit 100] [--ledger .claude/verdicts.log]
...
def main():
    ...
    prs = json.loads(gh(["pr", "list", "--state", "merged",
                         "--limit", str(a.limit), "--json",
                         "number,title,body,mergedAt,files"], a.repo))
